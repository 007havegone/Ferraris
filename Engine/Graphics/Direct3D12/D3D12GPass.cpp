...
namespace ferraris::graphics::d3d12::gpass {
...
math::u32v2					dimension{ initial_dimensions };
...
create_buffers(math::u32v2 size)
{
	...
	desc.Alignment = 0; // NOTE: 0 is same as 64KB (or 4MB for MSAA)
...
{
	assert(!gpass_root_sig && !gpass_root_sig);

	// Create GPass root signature
	using idx = gpass_root_param_indices;
	d3dx::d3d12_root_parameter parameters[idx::count]{};
	parameters[idx::root_constans].as_constants(3, D3D12_SHADER_VISIBILITY_PIXEL, 1);
	...
	struct {
		...
		d3dx::d3d12_pipeline_state_subobject_depth_stencil1			depth{ d3dx::depth_state.disable };
	}stream;
...
render(id3d12_graphics_command_list * cmd_list, const d3d12_frame_info & d3d12_info)
{
	...
	struct {
		...
		f32 height;
		u32 frame;
	}constants{ d3d12_info.surface_width, d3d12_info.surface_height, ++frame };

	using idx = gpass_root_param_indices;
	cmd_list->SetGraphicsRoot32BitConstants(idx::root_constans, 3, &constants, 0);
...
void
set_render_target_for_depth_prepass(id3d12_graphics_command_list* cmd_list)
{
...
void
set_render_target_for_gpass(id3d12_graphics_command_list* cmd_list)
{
