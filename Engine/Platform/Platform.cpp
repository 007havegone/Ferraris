...
LRESULT CALLBACK internal_window_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    return long_ptr
        ...
        : DefWindowProc(hwnd, msg, wparam, lparam); // default process
}
...
set_window_fullscreen(window_id id, bool is_fullscreen)
{
    ...
    if (info.is_fullscreen != is_fullscreen)
    {
        ...
        if (is_fullscreen)
        {
            ...
            info.style = 0;// do not need the border of window
...
create_window(const window_init_info* const init_info /* = nullptr*/)
{
    ...
    wc.lpszClassName = L"FerrarisWindow";
    ...
    const wchar_t* caption{ (init_info && init_info->caption) ? init_info->caption : L"Ferraris Game" };
    ...
    const s32 height{ (init_info && init_info->width) ? init_info->height : rc.bottom - rc.top };
    ...
    if (info.hwnd)
    {
        ...
        ShowWindow(info.hwnd, SWP_SHOWWINDOW);
